...
export class CtcSubmission {
  ...
  private readonly openedHintOrders = new Set<number>()

  getCachedHint(hintId: string): CtcHint | null | undefined {
    return this.hintCache.get(hintId)
  }

  canOpenHint(h: CtcHint): boolean {
    const order = h.order ?? 0
    if (order === 1) return true
    return this.openedHintOrders.has(order - 1)
  }
  ...
  onHintOpened(h: CtcHint): void {
    ...
    if (!this.canOpenHint(h)) return
    this.openedHintOrders.add(order)
