...
export class CtcAuthGuard implements CanActivate {
  ...
  canActivate(): Observable<boolean | UrlTree> {
    ...
    if (snap.status === 'authenticated') return of(true)
    if (snap.status === 'unauthenticated') return of(this.router.createUrlTree(['/ctc']))
    return this.session.loadMe().pipe(
      map((user) => (user ? true : this.router.createUrlTree(['/ctc'])))
    )
  }
}
