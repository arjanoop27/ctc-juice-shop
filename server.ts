...
const ctcHttpProxy = createProxyMiddleware({
    target: 'http://localhost:4000/ctc',
    changeOrigin: true,
    ws: false,
    secure: false,
    logLevel: 'debug',
    onProxyReq: (proxyReq: any, req: Request, res: Response) => {
        console.log('[CTC PROXY][HTTP]', req.method, req.originalUrl)
    },
    onError: (err: Error, req: IncomingMessage, res: any) => {
        console.error('[CTC PROXY][HTTP ERROR]', req.method, req.url, err.message)
    }
})

const ctcWsProxy = createProxyMiddleware({
    target: 'http://localhost:4000',
    changeOrigin: true,
    ws: true,
    secure: false,
    logLevel: 'debug',
    onProxyReqWs: (proxyReq: any, req: IncomingMessage, socket: Socket, head: Buffer) => {
        console.log('[CTC PROXY][WS]', req.method, req.url)
    },
    onError: (err: Error, req: IncomingMessage, res: any) => {
        console.error('[CTC PROXY][WS ERROR]', req.method, req.url, err.message)
    }
})
...
restoreOverwrittenFilesWithOriginals().then(() => {
    ...
    app.use(['/.well-known/security.txt', '/security.txt'], securityTxt({
        ...
        'Preferred-Languages': [...new Set(locales.map((locale: {
            key: string
        }) => locale.key.substr(0, 2)))].join(', '),
    ...
    app.use('/.well-known', serveIndexMiddleware, serveIndex('.well-known', {
        icons: true,
        view: 'details'
    }))
...
export async function start(readyCallback?: () => void) {
    ...
    server.on('upgrade', (req: IncomingMessage, socket: Socket, head: Buffer) => {
        if (req.url?.startsWith('/ctc/socket.io')) {
            console.log('[CTC PROXY][UPGRADE]', req.method, req.url)
            ctcWsProxy.upgrade(req, socket, head)
        }
    })
